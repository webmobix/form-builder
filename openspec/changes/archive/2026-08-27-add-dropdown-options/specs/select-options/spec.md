...
Lets form builders define the set of choices a Dropdown (`select`) field offers, and lets form fillers pick from those choices via a native `<select>` control whose selected value is submitted with the form.
