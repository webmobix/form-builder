...
A `designType === 'paragraph'` element SHALL carry its body copy in an optional `text: string` property. The `label` property SHALL remain the canvas row title (a short summary). The `text` property SHALL be the multi-line prose rendered in the live form and edited via a textarea in the inspector.
...
#### Scenario: Paragraph label is the canvas row title
- **WHEN** a paragraph element is rendered in the canvas
- **THEN** the canvas row uses the element's `label` as its row title, not its `text`
...
The canvas SHALL render a design-only element as a row whose body reflects its `designType`: a heading row shows the `label` styled as a heading; a paragraph row shows the `label` (with the `text` body preview truncated); a row container row shows the `label` and a visual representation of its columns and children. Design-only rows SHALL be selectable like data rows and SHALL emit `wbFieldSelected` with the full element (including `children` for row containers).

#### Scenario: Heading row renders the label as a heading
- **WHEN** the canvas renders a `designType: 'heading'` element
- **THEN** the row body shows the element's `label` in a heading style distinct from data rows

#### Scenario: Paragraph row renders the label
- **WHEN** the canvas renders a `designType: 'paragraph'` element
- **THEN** the row body shows the element's `label` as the row title

#### Scenario: Row container row renders its columns and children
- **WHEN** the canvas renders a `designType: 'row'` element with `columns: 2` and children
- **THEN** the row body shows a flex row with one column per `columns` entry, each column showing its child rows

#### Scenario: Selecting a design-only row emits wbFieldSelected
- **WHEN** the user clicks a design-only row
