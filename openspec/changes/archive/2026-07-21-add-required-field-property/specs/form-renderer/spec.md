...
The `wb-form-renderer` component SHALL accept a `fields: FieldMeta[]` prop and render one `wb-form-field` child per entry inside a `<form>` element in its shadow DOM. For each field it SHALL forward the entry's `type`, `label`, `subtype`, `restrictions`, and `required` to the corresponding `wb-form-field` props. The renderer SHALL derive each rendered field's submission `name` as `field.<id>` using the entry's stable `id`.
